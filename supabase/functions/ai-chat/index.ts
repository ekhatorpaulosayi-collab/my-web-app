...
serve(async (req) => {
  ...
  try {
    ...
    // SAFEGUARD 3: Validate AI response stayed on-topic (skip for help and onboarding contexts)
    // Help and onboarding contexts are about Storehouse features by definition, so they don't need strict validation
    if (contextType !== 'help' && contextType !== 'onboarding' && !validateResponse(aiResponse, message)) {
    ...
    return jsonResponse({
      ...
      quotaInfo: quotaInfo,
      conversationId: conversation?.id || null,  // CRITICAL: Include conversation ID for polling
      context: {
