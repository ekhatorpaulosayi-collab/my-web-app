...
serve(async (req) => {
  ...
  try {
    ...
    if (userId && store.user_id !== userId) {
      ...
      return new Response(
        JSON.stringify({ error: 'Unauthorized - not store owner' }),
    ...
    const { data: tierData } = await supabase
      ...
      .select('name')
      .eq('id', subscription?.tier_id || 1) // Default to tier 1 (Free) if no subscription
      .single();
    ...
    // Query sales data for current period
    ...
    // Query sales data for previous period
    ...
    const previousTotalSales = previousSales?.reduce((sum, sale) => sum + (sale.total || 0), 0) || 0;

    // Query top products sold
    ...
    topProducts?.forEach(item => {
      ...
      product.revenue += item.quantity * item.products.price;
    });
