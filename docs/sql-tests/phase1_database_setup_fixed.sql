-- PHASE 1: DATABASE FOUNDATION FOR CHAT SYSTEM (FIXED)
...
DROP POLICY IF EXISTS "Everyone can view conversations" ON ai_chat_conversations;
DROP POLICY IF EXISTS "Authenticated users can create conversations" ON ai_chat_conversations;
...
USING (
  ...
  OR
  auth.uid()::text IN (
    SELECT user_id::text FROM stores WHERE id::text = ai_chat_conversations.store_id::text
  )
);
