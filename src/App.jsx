...
import WaitingCustomerBanner from './components/dashboard/WaitingCustomerBanner.tsx';
import { checkForWaitingCustomers, playNotificationChime, requestNotificationPermission } from './services/ownerNotificationService.ts';
import { isSoundBlocked, enableAudio, getAudioState } from './utils/notificationSound.ts';
...
function App() {
  ...
  // Source of truth for the dashboard header business name:
  // stores.business_name → users.business_name → "My Store" (never "Storehouse").
  // Both reads survive logout/login because they hit Supabase, not localStorage.
  const { store: headerStore } = useStore(currentUser?.uid);
  const { user: headerUser } = useUser(currentUser);
  const headerBusinessName =
    headerStore?.business_name ||
  ...
  // Waiting customers notification state
  ...
  const [seenConversations] = useState(() => new Set());
  ...
  useEffect(() => {
    ...
    const checkAudioState = () => {
      ...
      console.log('🔊 [AudioContext] Current state:', state, 'Blocked:', blocked);
      setAudioBlocked(blocked);

      // Show prompt if audio is blocked and we have waiting customers
      if (blocked && waitingCustomers.length > 0) {
    ...
    const handleAudioEnabled = () => {
      console.log('🔊 [AudioContext] Audio enabled!');
      setAudioBlocked(false);
      setShowSoundPrompt(false);
    };

    window.addEventListener('audioContextEnabled', handleAudioEnabled);

    // Check audio state periodically
    const interval = setInterval(checkAudioState, 5000);

    return () => {
      window.removeEventListener('audioContextEnabled', handleAudioEnabled);
      clearInterval(interval);
    };
  }, [waitingCustomers.length]);

  // Handler to enable audio
  const handleEnableAudio = async () => {
    console.log('🔊 [AudioContext] User clicked to enable audio');
  ...
  // Poll for waiting customers (CRITICAL FIX)
  useEffect(() => {
    console.log('🎯 [APP-DEBUG-1] Notification useEffect triggered. currentUser:', currentUser?.uid);
    if (!currentUser) {
      console.log('🎯 [APP-DEBUG-2] No currentUser, skipping notification setup');
      return;
    }

    console.log('🎯 [APP-DEBUG-3] Setting up notification system for user:', currentUser.uid);
    ...
    const checkWaiting = async () => {
      console.log('🎯 [APP-DEBUG-4] Polling for waiting customers at:', new Date().toISOString());
      try {
        const waiting = await checkForWaitingCustomers(currentUser.uid);
        console.log('🎯 [APP-DEBUG-5] Poll result: found', waiting.length, 'waiting customers');

        // Check for NEW conversations and play sound only for new ones
        const now = Date.now();
        ...
        for (const conv of waiting) {
          ...
          // Check if conversation is not stale (created within last 30 minutes)
          const convTime = new Date(conv.created_at).getTime();
          if (convTime < thirtyMinutesAgo) {
            console.log('🎯 [APP-DEBUG-STALE] Skipping stale conversation:', conv.id);
            continue;
          }

          // Play sound for NEW conversation if not muted and enough time has passed
          const currentMuted = localStorage.getItem('notifications-muted') === 'true';
          if (!currentMuted && now - lastSoundPlayed > 5000) { // Min 5 seconds between sounds
            console.log('🎯 [APP-DEBUG-NEW] Playing sound for NEW customer:', conv.id);
            playNotificationChime();
            setLastSoundPlayed(now);
            break; // Only play one sound per check
          }
        }

        setWaitingCustomers(waiting);
        console.log('🎯 [APP-DEBUG-6] State updated with waiting customers');
      } catch (error) {
        console.error('❌ [APP-DEBUG-7] Error checking waiting customers:', error);
      }
    };

    // Check immediately
    console.log('🎯 [APP-DEBUG-8] Running initial check');
    checkWaiting();

    // Poll every 10 seconds
    console.log('🎯 [APP-DEBUG-9] Setting up 10-second polling interval');
    const pollInterval = setInterval(checkWaiting, 10000);

    // Removed the soundInterval that played sound repeatedly

    return () => {
      console.log('🎯 [APP-DEBUG-12] Cleaning up notification intervals');
      clearInterval(pollInterval);
    };
  }, [currentUser, lastSoundPlayed]);
    ...
    try {
      ...
      if (currentUser?.uid) {
        ...
        const supabaseSale = {
          ...
          quantity: qty,
          unit_price: sellKobo / 100, // FIX: Convert kobo to naira (was missing division)
          total_amount: totalKobo / 100, // FIX: Convert kobo to naira
          final_amount: totalKobo / 100, // FIX: Convert kobo to naira
          ...
          payment_method: formData.isCreditSale ? 'credit' : (formData.paymentMethod || 'cash'),
          payment_status: formData.isCreditSale ? 'pending' : 'paid',
          amount_paid: formData.isCreditSale ? 0 : totalKobo / 100, // FIX: Convert kobo to naira
          amount_due: formData.isCreditSale ? totalKobo / 100 : 0, // FIX: Convert kobo to naira
        ...
        // REMOVED DUPLICATE SAVE: This was causing double entries
        // The sale is already saved to Supabase at line 3101 with correct kobo/naira conversion
        // Keeping this second save would cause duplicate sales in the database
        console.log('[App] ✅ Sale already saved to cloud in earlier operation (line 3101)');
      } else {
  ...
  return (
    <div className="app container">
      ...
      {waitingCustomers.length > 0 && !isBannerDismissed && (
        <>
          {console.log('🎯 [APP-DEBUG-14] Rendering WaitingCustomerBanner with', waitingCustomers.length, 'waiting customers')}
          <WaitingCustomerBanner
            waitingCount={waitingCustomers.length}
            isMuted={isMuted}
            onToggleMute={() => {
              console.log('🎯 [APP-DEBUG-15] Toggle mute clicked. Current:', isMuted);
              const newMuted = !isMuted;
              setIsMuted(newMuted);
              localStorage.setItem('notifications-muted', String(newMuted));
            }}
            onGoToChats={() => {
              console.log('🎯 [APP-DEBUG-16] Go to chats clicked');
              navigate('/conversations');
            }}
            onDismiss={() => {
              console.log('🎯 [APP-DEBUG-17] Dismiss banner clicked');
