...
export default function AIChatWidget({
  ...
  const [storeData, setStoreData] = useState<{
    id: string;
    user_id?: string; // Store owner's ID for notifications
  ...
  const checkBusinessHours = (businessHours: any, daysOfOperation: any): boolean => {
    try {
      ...
      if (!businessHours || !daysOfOperation) {
        // No business hours configured = assume always available for human takeover
        return true;
      }
      ...
      const fetchStoreData = async () => {
        try {
          ...
          const { data, error } = await supabase
            .from('stores')
            .select('id, user_id, whatsapp_number, wa_fallback_minutes, business_hours, days_of_operation')
  ...
  const requestHumanAgent = async () => {
    console.log('🚨 [CHAT-DEBUG-1] Talk to Store Owner button clicked!');
    console.log('🚨 [CHAT-DEBUG-2] Customer requesting human agent');

    // Get session ID for tracking
    const sessionId = sessionStorage.getItem('chat_session_id');
    console.log('🚨 [CHAT-DEBUG-3] SessionId:', sessionId);
    console.log('🚨 [CHAT-DEBUG-4] ConversationId:', conversationId);
    console.log('🚨 [CHAT-DEBUG-5] StoreId:', storeData?.id);
    console.log('🚨 [CHAT-DEBUG-6] Store user_id:', storeData?.user_id);

    if (!sessionId || !conversationId) {
      console.error('🚨 [CHAT-DEBUG-7] No session/conversation to request agent for');
      return;
    }

    try {
      // Update conversation status to 'requested' with timestamp and ensure is_agent_active is false
      const updateData: any = {
        takeover_status: 'requested',
        waiting_for_owner_since: new Date().toISOString(),
        is_agent_active: false  // CRITICAL: Ensure this is false when requesting owner
      };

      console.log('🚨 [CHAT-DEBUG-8] Setting waiting_for_owner_since to:', updateData.waiting_for_owner_since);
      console.log('🚨 [CHAT-DEBUG-9] Setting is_agent_active to false');
      console.log('🚨 [CHAT-DEBUG-10] About to update conversation with data:', updateData);
      ...
      if (updateError) {
        console.error('🚨 [CHAT-DEBUG-11] Error requesting agent:', updateError);
        return;
      }

      console.log('🚨 [CHAT-DEBUG-12] Update successful! waiting_for_owner_since has been set');

      // Send push notification to store owner via the ai-chat edge function (non-blocking).
      // We can't call send-push-notification directly anymore — its public HTTP entry
      // requires a user JWT (security audit M11) and storefront customers don't have one.
      // ai-chat acts as the relay using the service-role key server-side.
      try {
        console.log('🚨 [CHAT-DEBUG-13] Sending push notification to store owner via ai-chat relay');

        const lastCustomerMessage = messages
          .filter(m => m.role === 'user')
          .slice(-1)[0]?.content || 'Customer needs help';

        const relayPayload = {
          // Required by ai-chat's existing schema, even though this branch
          // short-circuits before the message-validation gate.
          message: '[owner-notification-relay]',
          contextType: 'storefront' as const,
          storeSlug,
          sessionId: sessionId || undefined,
          // Owner-notification relay fields (added 2026-05-01).
          requestOwnerNotification: true,
          conversationId,
          customerName: customerName || 'Customer',
          messagePreview: lastCustomerMessage.substring(0, 100),
        };

        console.log('🚨 [CHAT-DEBUG-14] Owner-notification relay payload:', relayPayload);
        ...
        fetch(`${supabaseUrl}/functions/v1/ai-chat`, {
          ...
          console.log('🚨 [CHAT-DEBUG-15] Owner-notification relay response:', response.status);
          return response.json();
        }).then(data => {
          console.log('🚨 [CHAT-DEBUG-16] Owner-notification relay result:', data);
        }).catch(err => {
          console.error('🚨 [CHAT-DEBUG-17] Owner-notification relay failed:', err);
          // Don't fail the main flow if push fails
        });
      } catch (err) {
        console.error('🚨 [CHAT-DEBUG-18] Error sending owner notification:', err);
      ...
      console.log('🚨 [CHAT-DEBUG-19] Human agent requested, system message will be fetched via polling');
