...
export default function AIChatWidget({
  ...
  const requestHumanAgent = async () => {
    ...
    try {
      ...
      // Send push notification to store owner (non-blocking)
      try {
        console.log('🚨 [CHAT-DEBUG-13] Sending push notification to store owner');

        // Get the last customer message for preview
        ...
        console.log('🚨 [CHAT-DEBUG-14] Push notification payload:', notificationPayload);
        ...
        fetch(`${supabaseUrl}/functions/v1/send-push-notification`, {
          ...
          console.log('🚨 [CHAT-DEBUG-15] Push notification response:', response.status);
          return response.json();
        }).then(data => {
          console.log('🚨 [CHAT-DEBUG-16] Push notification result:', data);
        }).catch(err => {
          console.error('🚨 [CHAT-DEBUG-17] Push notification failed:', err);
          // Don't fail the main flow if push fails
        });
      } catch (err) {
        console.error('🚨 [CHAT-DEBUG-18] Error sending push notification:', err);
