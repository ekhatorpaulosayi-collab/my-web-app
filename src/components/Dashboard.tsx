...
export function Dashboard({
  ...
  // Also load user record so business name can fall back to users.business_name
  // before the final "My Store" default (never "Storehouse").
  const { user: storeOwnerUser } = useUser(userId ? { uid: userId } : null);
  const storeSlug = store?.store_slug || '';
  const businessName = store?.business_name || storeOwnerUser?.business_name || 'My Store';
