...
const BusinessInsights: React.FC = () => {
  ...
  useEffect(() => {
    const checkAccess = async () => {
      ...
      if (stores && stores.length > 0) {
        ...
        // Fallback: legacy single-store setups stored store row with id == user uid
  ...
  const generateSummary = async () => {
    ...
    try {
      // Send the user's JWT (not the anon key) so the edge function can
      // verify ownership. Previously we sent the anon key, which made the
      // edge function's `auth.getUser()` return null and bypassed the
      // ownership check entirely (security audit H1 / M5).
