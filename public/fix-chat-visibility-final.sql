-- Fix Chat Agent Takeover: Customer Message Visibility (FINAL VERSION)
-- This SQL fixes the issue where customers cannot see agent messages after takeover
-- FIXED: Drops all existing versions of send_agent_message function first
...
-- 4. DROP ALL EXISTING VERSIONS of send_agent_message function
...
DROP FUNCTION IF EXISTS send_agent_message(p_conversation_id UUID) CASCADE;
DROP FUNCTION IF EXISTS send_agent_message(p_conversation_id UUID, p_message TEXT) CASCADE;
DROP FUNCTION IF EXISTS send_agent_message(p_conversation_id UUID, p_message TEXT, p_agent_id UUID) CASCADE;

-- 5. Create the send_agent_message function
