...
self.addEventListener('push', (event) => {
  ...
  let notificationData = {
    ...
    body: 'A customer needs help in your store chat',
  ...
  if (event.data) {
    try {
      ...
      if (payload.conversationId) {
        notificationData.data.conversationId = payload.conversationId;
        notificationData.tag = `conversation-${payload.conversationId}`;
      }
      if (payload.customerName) {
        notificationData.title = `${payload.customerName} is waiting!`;
      }
      if (payload.messagePreview) {
        notificationData.body = payload.messagePreview;
      }
    } catch (err) {
...
self.addEventListener('notificationclick', (event) => {
  ...
  // Open the dashboard conversations page
  let targetUrl = '/conversations';

  // If we have a conversation ID, open that specific conversation
  if (event.notification.data && event.notification.data.conversationId) {
    targetUrl = `/conversations?id=${event.notification.data.conversationId}`;
  }
  ...
  event.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true })
      .then((clientList) => {
        // Check if we already have a dashboard tab open
        for (const client of clientList) {
          const url = new URL(client.url);
          if (url.pathname === '/conversations' || url.pathname === '/dashboard') {
            // Focus existing tab and navigate to conversation
